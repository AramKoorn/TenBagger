...
class Portfolio(PortfolioCrypto):
    ...
    def weighted_staking_rewards(self):
        return (
            self.df[self.df.sector == "Crypto"].staking_rewards.sum()
            / self.df[self.df.sector == "Crypto"].value.sum()
            * 100
            if not None
            else 0
        )
    ...
    def weighted_dividend_yield(self):
        return (
            self.df[self.df.sector != "Crypto"].dividends.sum()
            / self.df[self.df.sector != "Crypto"].value.sum()
            * 100
            if not None
            else 0
    ...
    def weighted_yield(self):
        return (
            self.df.passive_income.sum() / self.df.value.sum() * 100 if not None else 0
        )
    ...
    def unification(self):
        ...
        df["price"] = df.apply(
            lambda x: self.c.convert(x.currency, self.env["CURRENCY"], x.price), axis=1
        )
        ...
        df[["dividends", "passive_income"]] = df[
            ["dividends", "staking_rewards"]
        ].fillna(0)
