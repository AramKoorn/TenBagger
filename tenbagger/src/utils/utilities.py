...
class Ticker:
    ...
    def history_prices(self, dates):
        ...
        return {
            date: self.ticker.history(date).Close[0] for i, date in enumerate(dates)
        }
    ...
    def overview_stonks(self, info):
        ...
        overview = {
            ...
            if info["earningsQuarterlyGrowth"] is not None
            else info["earningsQuarterlyGrowth"],
    ...
    def overview(self):
        ...
        if info["quoteType"]["quoteType"] == "CRYPTOCURRENCY":
            ...
            overview = self.overview_stonks(
                {**info["defaultKeyStatistics"], **info["summaryDetail"]}
            )
...
def order_by_month(df, col):
    ...
    mp = {
        datetime.date(1900, month, 1).strftime("%B"): i
