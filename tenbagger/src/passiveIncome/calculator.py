...
class PassiveIncomeCalculator:
    def __init__(self, port):
        ...
        self.payout = pd.concat(
            {
                month: self.res.query(f"month == {month}")
                for month in set(self.res.month)
            }
        ...
        for token in set(self.crypto.ticker):
            monthly_payout = (
                (self.crypto.loc[self.crypto.ticker == token, "apy"].values[0])
                * self.crypto.loc[self.crypto.ticker == token, "price"].values[0]
            ) / 12
            range_months = list(range(1, 13))
            crypto_payout = {
                "ticker": [token] * 12,
                "staking_rewards": [monthly_payout] * 12,
                "month": range_months,
            }
    ...
    def calulate(
        ...
        if only_dividend_stocks:
            ...
            df["percentage"] = (
                np.float32(df["percentage"].apply(lambda x: x.replace("%", ""))) / 100
            )
        ...
        for m in range(0, n + 1):
            ...
            for ticker in set(self.crypto_payout.ticker):
                if not only_dividend_stocks:
                    amount = df.loc[df.ticker == ticker, "amount"].values[0]
                    sr = (
                        df.loc[df.ticker == ticker, "apy"].values[0]
                        * df.loc[df.ticker == ticker, "price"].values[0]
                    ) / 12
                    df.loc[df.ticker == ticker, "monthly_staking_rewards"] = amount * sr
                else:
                    df["monthly_staking_rewards"] = 0

            df["dripping"] = np.where(
                df.monthly_dividend > 0, df.monthly_dividend / df.price, 0
            )
            df["drip_staking_rewards"] = np.where(
                df.monthly_staking_rewards > 0, df.monthly_staking_rewards / df.price, 0
            )
            df["amount"] += (
                df["adding_units"] + df["dripping"] + df["drip_staking_rewards"]
            )
            ...
            data = {
                ...
                "total_passive_income": [
                    df.monthly_dividend.sum() + df.monthly_staking_rewards.sum()
                ],
            ...
            self.df_report = pd.concat(
                [self.df_report, pd.DataFrame.from_dict(data)], axis=0
            )
